    ...
    setTimeout(() => {
      try {
        ...
        if (months >= maxMonths && netDifference < targetImpact) {
          setError(
            `O impacto de ${formatCurrency(targetImpact)} não foi atingido em 30 anos. A diferença máxima atingida foi de ${formatCurrency(netDifference)}.`
          );
          setHistory([]);
        } else {
      ...
      {isLoading && (
        <div className="mt-8 text-center">
          ...
          <p className="text-gray-600 mt-4">
            Simulando mês a mês, isso pode levar alguns segundos...
          </p>
        </div>
      )}
        ...
        <div className="mt-8 max-w-2xl p-6 bg-green-50 border-2 border-green-300 rounded-lg shadow-lg">
          ...
          <div className="text-center mb-6">
            <p className="text-lg text-gray-700">
              Tempo para atingir o impacto de{' '}
              <span className="font-bold">{formatCurrency(targetImpact)}</span>:
            </p>
            <p className="text-4xl font-bold text-blue-700 mt-2">
              {result.years} {result.years === 1 ? 'ano' : 'anos'} e {result.remainingMonths}{' '}
              {result.remainingMonths === 1 ? 'mês' : 'meses'}
            </p>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center p-3 bg-blue-100 rounded-lg border border-blue-300 mt-2">
                ...
                <span className="font-bold text-lg text-blue-800">
                  {formatCurrency(result.finalDifference)}
                </span>
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-sm text-left text-gray-700">
              <thead className="text-xs text-gray-700 uppercase bg-gray-100">
                <tr>
                  ...
                  <th scope="col" className="px-4 py-3 bg-green-50">
                    LCI Início
                  </th>
                  <th scope="col" className="px-4 py-3 bg-green-50">
                    LCI Rend.
                  ...
                  <th scope="col" className="px-4 py-3 bg-purple-50">
                    CDB Início
                  </th>
                  <th scope="col" className="px-4 py-3 bg-purple-50">
                    CDB Rend.
                  ...
                  <th scope="col" className="px-4 py-3 bg-blue-50">
                    Diferença
                  </th>
                </tr>
              </thead>
              <tbody>
                {history.map((row) => (
                  <tr key={row.month} className="bg-white border-b hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-600">{formatCurrency(row.contribution)}</td>
                    <td className="px-4 py-3 bg-green-50">{formatCurrency(row.lciStart)}</td>
                    ...
                    <td className="px-4 py-3 bg-purple-50">{formatCurrency(row.cdbStart)}</td>
